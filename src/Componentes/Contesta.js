...
function Contesta({
    ...
    const Nointeresa = async() => {
        const result = await axios.post(
            "https://app.soluziona.cl/API_QA/Peru/Call/api/Ventas_CRM/Call/ConectaClienteNoInteresa", { dato: "20367002" }, { headers: { Authorization: `Bearer ${clave}` } }
        );
    ...
    async function GuardarRegistro() {
        ...
        try {
            const result = await axios.post(
                "https://app.soluziona.cl/API_QA/Peru/Call/api/Ventas_CRM/Call/GuardaGestion", { dato: id }, { headers: { Authorization: `Bearer ${token}` } }
            );
    ...
    const Company = async(company) => {
        const result = await axios.post(
            "https://app.soluziona.cl/API_QA/Peru/Call/api/Ventas_CRM/Call/ConectaDetalle", { dato: company }, { headers: { Authorization: `Bearer ${clave}` } }
        );
    ...
    const DataOcupacion = async() => {
        const result = await axios.post(
            "https://app.soluziona.cl/API_v1_prod/CallSouthPeru/APIVentas_Call/api/Ventas/Call/Profesiones", { dato: null }, { headers: { Authorization: `Bearer ${clave}` } }
        );
    ...
    return ( <
        ...
        <
        p > Buenos días / tardes, me podría comunicar con el(la) señor(a) { nombreCliente }, Cómo le va mucho gusto!Encantado / a de saludarle mi nombre es XXXX le llamamos de Banco Ripley por encargo de Chubb Seguros Perú. <
        /p> <
        /div> <
        div >
...

        {
            select_si_conecta_llamada && ( <
                ...
                <
                label
                for = "observacion" > ¿Entonces Sr.…Siendo el día de hoy(DD / MM / AAAA) acepta la contratación de“ Sonríe Seguro”, con un cargo fijo mensual de S / .29.90 en su tarjeta de crédito Ripley ? -Esperar respuesta afirmativa del cliente(Sí - Sí acepto). <
                /label> <
                ...
                p >
                <
                strong > Agendo : < /strong> Por supuesto (Primer nombre del cliente) se programará la llamada para el día (validar fecha con cliente). Muchas gracias, buenos días/buenas
...

                {
                    select_conecta_llamada_pregunta_interesa === "3" && ( <
                        section >
                        <
                        Terceros conecta = { selectLlamada }
                    ...
                    select_conecta_llamada_pregunta_interesa === "2" && ( <
                        ...
                        <
                        div id = "vw_script_cliente_nointeresa"
                        className = "" >
                        <
                        p >
                        <
                        ...
                        <
                        select id = "select_conecta_llamada_pregunta_no_interesa"
                        className = "form-select cliente"
                        disabled = {
                            select_conecta_llamada_pregunta_interesa !== "2"
                        }
...

                        {
                            select_conecta_llamada_pregunta_no_interesa !== "0" && ( <
                                ...
                                <
                                button className = "btn text-white guardar"
                    ...
                    select_conecta_llamada_pregunta_interesa === "1" && ( <
                        div id = "vw_script_cliente_valida"
                        class = "" >
                        <
                        div className = " row m-2 rounded p-2"
                        style = {
                            { background: "#EDEDED" } } >
                        <
                        h4 >
                        ...
                        onChange = {
                            () => setBuenEstadoSalud("NO") }
                        /> <
                        label className = "m-1" > NO < /label> <
                        /div> {
                            buenEstadoSalud === "NO" && ( <
                                p >
                                Salvedad: Recuerde Sr. / Sra.que le vamos a cubrir por cualquier accidente que no sea consecuencia de alguna preexistencia. <
                                /p>
                            )
                        } <
                        ...
                        <
                        /label> <
                        select className = "form-control form-select my-2 cliente"
                        id = "ocupacion"
                        ...
                        div className = " m-4 " >
                        <
                        p > ¡Felicitaciones, desde este momento ya se encuentra protegido con Sonríe Seguro de Chubb Seguros…!
                        <
                        /p> <
                        p >
                        ...
                        <
                        label
                        for = "observacion" > ¿Don / Sra.XXXX, Usted desea que le enviemos su Póliza a través de su correo electrónico ?
                        <
                        /label> <
                        select value = { selectConectaLlamadaPreguntaConfirma }
                        onChange = {
                            (e) =>
...

                        {
                            ...
                            selectConectaLlamadaPreguntaConfirma === "2" && ( <
                                ...
                                <
                                p >
                                Te estaremos enviando la Solicitud - Certificado a tu domicilio en 15 días calendario, contados a partir del día de hoy Revalidar(LA DIRECCIÓN:
...

                {
                    (selectConectaLlamadaPreguntaConfirma == "1" ||
                        ...
                        <
                        div id = "vw_script_cliente_despedida"
                        className = "" >
                        <
                        div className = "" >
                        <
